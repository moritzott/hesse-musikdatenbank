...
def write_results_to_file(res, words, start, end):
    ...
    # Dateiname setzt sich zusammen aus der aktuellen Zeit und 'ergebnisse-'
    date = datetime.datetime.now()
    #print(str(date.strftime("%d")))
    day = str(date.strftime("%d"))
    month = str(date.strftime("%B"))
    year = str(date.strftime("%Y"))
    hour = str(date.strftime("%H"))
    minutes = str(date.strftime("%M"))
    seconds = str(date.strftime("%S"))
    filename = f"ergebnisse-{day}-{month}-{year}--{hour}H{minutes}MIN{seconds}SEC.txt"
